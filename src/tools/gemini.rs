...
pub fn run_workspace_command(prompt: &str) -> Result<Option<String>, String> {
    ...
    let result: GeminiSessionOutput =
        serde_json::from_str(&stdout[json_start..]).map_err(|e| e.to_string())?;
    ...
    {
        return Err(
            "Insufficient Google Workspace permissions. Please run 'gemini login' manually."
                .to_string(),
        );
    }
